...
async def create_geofence(
    ...
    geofence = await GeofenceService(db).create_geofence(
...
async def list_geofences(
    ...
    rows = await GeofenceService(db).list_geofences(current_user, child_id)
...
async def get_geofence(
    ...
    geofence = await GeofenceService(db).get_geofence(current_user, geofence_id, "view")
    return success(_serialize(geofence))
...
async def update_geofence(
    ...
    geofence = await GeofenceService(db).update_geofence(
...
async def delete_geofence(
    ...
    await GeofenceService(db).delete_geofence(current_user, geofence_id)
