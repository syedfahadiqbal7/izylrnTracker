"""Sound Around (F11) — request-path service (Sprint 5 Slice 1).

Issues a `MONITOR` SIM command to the child's watch so it silently calls the requesting
parent back for ambient audio (CLAUDE.md §3.12 — no media server). The backend's job is
to **gate, issue, and log**; the audio itself flows watch→phone over the SIM and is never
observable here (so `audio_sessions.duration_s` stays NULL).

Gates (all enforced before any command is dispatched, cheapest first):
  1. **Authorization** — caller must be a family member with ``can_call`` (else 404/403).
  2. **Tier** — Basic+ feature, counted over the child's **primary parent** (like the
     geofence/emergency limits); Free → 402.
  3. **Watch online** — there must be a paired watch with a Traccar id that is currently
     online in Redis (`device:{id}:online`); else 404 (no watch) / 409 (offline).
  4. **Daily quota** — `sound_around_daily_limit` per child per day, tracked in Redis
     `sound_sessions:{child}` and expiring at the next midnight in the primary parent's tz.

Quota is consumed only on a **successful dispatch** (Traccar accepted the command), so a
failed dispatch (502) doesn't burn the parent's daily allowance.
"""
...
# Sound Around is a Basic+ feature (Premium + School inherit it).
SOUND_AROUND_TIERS = {"basic", "premium", "school"}


class SoundAroundService:
    ...
    def _enforce_tier(self, tier: str) -> None:
        if tier not in SOUND_AROUND_TIERS:
            raise APIException(
                402, "SOUND_AROUND_REQUIRES_BASIC",
                "Upgrade to Basic plan to use Sound Around",
            )
    ...
    async def _online_watch(self, child_id: uuid.UUID) -> Device:
        ...
        409 if one exists but isn't online (Sound Around needs a live watch)."""
        watches = (
            await self.db.execute(
                select(Device).where(
        ...
        for device in watches:
            ...
            if await self.redis.get(rk.device_online(device.id)):
                return device
        raise APIException(
            409, "WATCH_OFFLINE", "The watch is offline — Sound Around needs the watch online"
        )
