...
- **Done & on `main`:** Sprint 0 (infra, 33-table schema, partitioning) · Sprint 1 (Auth/User/Children/Family — PR #1) · Sprint 2 (real-time location pipeline Flow A — PR #2). **144 tests.** Per-slice detail in `ARCHITECTURE.md` §7–8.
- **Next:** Sprint 3 — Geofences (Flow B): zone CRUD, schedule, circle/polygon breach detection + debounce, School Mode. **Design proposed, awaiting confirmation — start at Slice 1 (CRUD) once decisions signed off.** Optional un-built: `GET /children/{id}/location/latest`.
