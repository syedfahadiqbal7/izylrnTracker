...
- **Done & on `main`:** Sprint 0 (infra, 33-table schema, partitioning) · Sprint 1 (Auth/User/Children/Family — PR #1) · Sprint 2 (real-time location pipeline Flow A — PR #2) · Sprint 3 (geofences Flow B — zone CRUD, geometry, breach engine, School Mode). **214 tests.** Per-slice detail in `ARCHITECTURE.md` §7–9.
- **Next:** Sprint 4 — next in the Sprint Plan (S0–S10). Plan + key decisions first, then build slice-by-slice. Optional un-built: `GET /children/{id}/location/latest`.
