...
- **Done & on `main`:** Sprint 0 (infra, 33-table schema, partitioning) · Sprint 1 (Auth/User/Children/Family — PR #1) · Sprint 2 (real-time location pipeline Flow A — PR #2) · Sprint 3 (geofences Flow B — PR #3) · Sprint 4 (SOS Flow C + Alerts inbox — PR #4). **259 tests.** Per-slice detail in `ARCHITECTURE.md` §7–10.
- **Also on `main`:** Sprint 5 (Audio F11+F12 — PR #5) · Sprint 6 (Payments & Subscriptions + Celery — PR #6). **325 tests.** Detail in `ARCHITECTURE.md` §10a (audio) + §10b (payments).
- **Next:** next in the Sprint Plan (S0–S10); School web admin / bus tracking (React, §2 says Sprint 9+) or remaining Phase-1 backend items (Pickup F17, Safe Addresses, Share Link, Watch-Removed, Safe Routes, Chat). Plan + key decisions first, then build slice-by-slice. Optional un-built: `GET /children/{id}/location/latest`.
...
- **Deferred (by design):** none outstanding. (30-day soft-delete purge + partition roll-forward shipped Sprint 6 Celery; guardian-accepted FCM done Sprint 2 Slice 5.) Weekly PDF (F25) intentionally deferred to its own sprint (needs R2 + PDF rendering). Outbound payment *checkout* live calls await real Razorpay/Stripe test keys in `.env`.
