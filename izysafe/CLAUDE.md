...
- **On branch `sprint-5-audio` (PR open, not yet merged):** Sprint 5 Audio — Sound Around (F11) + Two-way Call (F12). **+27 tests (286 total).** Detail in `ARCHITECTURE.md` §10a.
- **Next:** after Sprint 5 merges — next in the Sprint Plan (S0–S10); School web admin / bus tracking or remaining Phase-1 items. Plan + key decisions first, then build slice-by-slice. Optional un-built: `GET /children/{id}/location/latest`.
